...
generated by the default Hammer DRC run file. However, for ASAP7, the Calibre tool 
need to run the technology's encrypted rule deck, so Hammer replaces its default 
DRC run file with an extra hook. This is a good example of how Hammer leverages its hook methods to enhance its capabilities. 
